...
### Install Packages
In my first data tidying project, I started by installing and loading all necessary packges needed to run the requested functions. 
...
### Wrangle Data
After saving the raw data files, I took some time to run the data frame #2's code, line by line, to get a better understanding as to what each line of code accomplishes.
...
I then proceeded to manually run data frame #1's code: separting a column that violated tidy data standards, adding a value column, grouping the data by year, summarizing it and then reordering it to align with data frame #2. 
...
I combined both data frames by row and converted the "n" column into a numeric variable so it could be plotted later.
```{r merge-data, warning=FALSE} 
## Now, combine the two data frames
df_total <- rbind(df_retail, df_department)
## Convert "n" to a numeric variable
df_total <- df_total %>% mutate(n=as.numeric(n))
```
